...
class Usuario
{
  ...
  public function findDeudorID()
  {
    $sql = "CALL findDeudorID({$this->getId()})";
    $usuarios = $this->db->query($sql);
    return $usuarios;
  }
  ...
  // {
  //   $sql = "CALL COUSERDEUDOR1(@a)";
  //   $rr = $this->db->query($sql);
  //   $all = [];
  //   while ($fila = mysqli_fetch_assoc($rr)) {
  //     array_push($all, $fila);
  //   }
  ...
  //   $allUsers = $countZona;
  //   //
  //   $porcenJZona = bcdiv(($countZona / $countZona) * 100, '1', 2);
  ...
  // public function activosDeudor(){
  //   $sql = "CALL activosDeudor(@a, @b)";
  //   $rr = $this->db->query($sql);
  //   return $rr;
  // }
}
